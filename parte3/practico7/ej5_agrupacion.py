...
# Definir la agrupación de valores
grupos = [
    [0, 1],
    [2],
    [3],
...
# Calcular las probabilidades pi
...
def probs_agrupacion(grupos, p):
    probabilidades = []
    for grupo in grupos:
        p_grupo = sum(prob_binomial(i, n_b, p) for i in grupo)
        # puedo usar sum(binom.pmf(i, n_b, p_hat) for i in grupo) en lugar de mi función definida
        probabilidades.append(p_grupo)
    '''
    # Mostrar probabilidades por grupo
    for i, p in enumerate(probabilidades):
        print(f"Probabilidad del grupo {i} ({grupos[i]}) = {p:.4f}")
    '''
    return probabilidades

pi = probs_agrupacion(grupos, p_hat)

# Calcular las frecuencias Ni
def frecuencias_discretas_agrupacion(grupos, muestra):
    # Inicializar conteo para cada grupo
    frecuencias = [0] * len(grupos)

    # Contar las frecuencias por grupo
    for valor in muestra:
        for idx, grupo in enumerate(grupos):
            if valor in grupo:
                ...
                break
    '''
    # Imprimir frecuencias agrupadas
    for i, freq in enumerate(frecuencias):
        print(f"Frecuencia del grupo {i} ({grupos[i]}) = {freq}")
    '''
    return frecuencias

Ni = frecuencias_discretas_agrupacion(grupos, muestra)
...
def p_valor_sim(Nsim, n, T):
    ...
    for i in range(Nsim):
        ...
        pi_sim = probs_agrupacion(grupos, p_sim)
        Ni_sim = frecuencias_discretas_agrupacion(grupos, x_gen)
